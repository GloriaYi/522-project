...
auc_scores = pd.read_csv("../results/tables/auc_scores.csv")
...
# AUC scores
auc_high = round(auc_scores["high risk"].iloc[0], 4)
auc_low = round(auc_scores["low risk"].iloc[0], 4)
auc_mid = round(auc_scores["mid risk"].iloc[0], 4)
...
We also used Deepchecks' `FeatureFeatureCorrelation` to check for unusually high correlations between pairs of features, which may indicate redundancy or multicollinearity in modeling. Multicollinearity is important to consider as it can introduce uncertainty in the model's coefficients and predictions. Threshold used: correlation < 0.95. Feature–feature correlations met the threshold. No pairs of features exhibited extremely high correlation that would require removal or special handling.
...
A confusion matrix and ROC curve were generated to visualize the model's performance on the test dataset. The model performed well overall and was particularly strong in identifying high-risk pregnancies.

![Crosstab of Actual vs Predicted Risk Levels](../results/figures/confusion_matrix.png){#fig-confusion-matrix fig-cap="Confusion matrix of model performance on test data." size="60%"}
